...
const START_DATE = DateTime.fromISO("2022-04-09");
...
export function Share({
  ...
  const shareText = useMemo(() => {
    ...
    const title = `#Kommunle #${dayCount} ${guessCount}/6 ${bestPercent}${difficultyModifierEmoji}`;
    ...
    return [title, guessString, "https://kommunle.se"].join("\n");
  ...
  return (
    ...
    >
      <button className="rounded font-bold border-2 p-1 uppercase bg-red-600 hover:bg-red-500 active:bg-red-700 text-white w-full">
        {t("share")}
      </button>
    </CopyToClipboard>
  );
